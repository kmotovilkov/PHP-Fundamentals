...
while (true) {
    ...
    if ($command == "Add") {
        $bandMembers = explode(", ", $tokens[2]);
        foreach ($bandMembers as $bandMember) {
            $bands[$band] [] = $bandMember;
        }
    } else if ($command == "Play") {
        $totalPlayTime += $tokens[2];
        if (!key_exists($band, $playTime)) {
            $playTime[$band] = $tokens[2];
        } else {
            $playTime[$band] += $tokens[2];
        }
    }
...
foreach ($playTime as $band => $time) {
    echo "$band -> $time" . PHP_EOL;

}
echo "$printBand " . PHP_EOL;
foreach (array_unique($bands[$printBand]) as $members) {
